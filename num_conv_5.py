def conv_num(num_str):
    ...
    for char in range(0, len(num_str)):
        ...
        if char == 0 and converted_char == 45:  # Checks if the first char is a minus sign
            ...
            pass
        elif 65 <= converted_char <= 70 or 97 <= converted_char <= 102:
            if x_flag is False:
                return None
        elif converted_char == 88 or converted_char == 120 and decimal_flag is False:
            x_flag_check = set_x_flag(sign_flag, char)
            if x_flag_check is True:
                x_flag = True
            else:
                return None
        else:
    ...
    final_num = determine_convert(x_flag, num_str)

    return final_num


def set_x_flag(sign_flag, char):
    x_check = False
...
def determine_convert(x_flag, num_str):
    if x_flag is True:
